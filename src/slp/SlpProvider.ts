...
export type SlpWatchBalanceCallback = (balance: SlpTokenBalance[]) => boolean | void;
...
export interface SlpProvider {
  ...
  SlpSpendableUtxos(cashaddr: string, ticker?: string, tokenId?: string): Promise<SlpUtxoI[]>;

  // token mint baton utxos
  SlpBatonUtxos(cashaddr: string, ticker?: string, tokenId?: string): Promise<SlpUtxoI[]>;

  // get all token balances
  SlpAddressTokenBalances(cashaddr: string, ticker?: string, tokenId?: string): Promise<SlpTokenBalance[]>;

  // get all slp transactions of this address
  SlpAddressTransactionHistory(cashaddr: string, ticker?: string, tokenId?: string): Promise<TxI[]>;

  // waits for next slp transaction to appear in mempool, code execution is halted
  SlpWaitForTransaction(cashaddr: string, ticker?: string, tokenId?: string): Promise<any>;

  // waits for a certain slp token balance to be available in this wallet, code execution is halted
  SlpWaitForBalance(value: BigNumber.Value, cashaddr: string, ticker: string, tokenId?: string): Promise<SlpTokenBalance>;
...
export function _convertBalanceBigNumbers(balances: SlpTokenBalance[]): SlpTokenBalance[] {
