...
import { SlpAllUtxosTemplate, SlpAddressTokenBalancesTemplate, SlpAddressTransactionHistoryTemplate, SlpWaitForTransactionTemplate, SlpBatonUtxosTemplate, SlpSpendableUtxosTemplate } from "./SlpDbTemplates";
import EventSource from "eventsource";
import BigNumber from "bignumber.js";
import { SlpCancelWatchFn, SlpProvider, SlpWatchBalanceCallback, SlpWatchTransactionCallback, _convertBalanceBigNumbers, _convertUtxoBigNumbers } from "./SlpProvider";
import axios from "axios";

const servers = {
  "mainnet": {slpdb: "https://slpdb.fountainhead.cash", slpsockserve: "https://slpsocket.fountainhead.cash"},
  "testnet": {slpdb: "https://slpdb-testnet.fountainhead.cash", slpsockserve: "https://slpsocket-testnet.fountainhead.cash"},
  "regtest": {slpdb: "http://localhost:12300", slpsockserve: "http://localhost:12301"}
};
...
export class SlpDbProvider implements SlpProvider {
  ...
  async SlpUtxos(cashaddr: string): Promise<SlpUtxoI[]> {
    return _convertUtxoBigNumbers((await this.SlpDbQuery(SlpAllUtxosTemplate(cashaddr))).g as SlpUtxoI[]);
  }

  // safe-spendable token utxos, without baton
  async SlpSpendableUtxos(cashaddr: string, ticker?: string, tokenId?: string): Promise<SlpUtxoI[]> {
    return _convertUtxoBigNumbers((await this.SlpDbQuery(SlpSpendableUtxosTemplate(cashaddr, ticker, tokenId))).g as SlpUtxoI[]);;
  }

  // token mint baton utxos
  async SlpBatonUtxos(cashaddr: string, ticker?: string, tokenId?: string): Promise<SlpUtxoI[]> {
    return _convertUtxoBigNumbers((await this.SlpDbQuery(SlpBatonUtxosTemplate(cashaddr, ticker, tokenId))).g as SlpUtxoI[]);
  }

  // get all token balances
  async SlpAddressTokenBalances(cashaddr: string, ticker?: string, tokenId?: string): Promise<SlpTokenBalance[]> {
    return _convertBalanceBigNumbers((await this.SlpDbQuery(SlpAddressTokenBalancesTemplate(cashaddr, ticker, tokenId))).g as SlpTokenBalance[]);
  }

  // get all slp transactions of this address
  async SlpAddressTransactionHistory(cashaddr: string, ticker?: string, tokenId?: string): Promise<TxI[]> {
    const response = await this.SlpDbQuery(SlpAddressTransactionHistoryTemplate(cashaddr, ticker, tokenId));
    return response.c.concat(response.u) as TxI[];
  }

  // waits for next slp transaction to appear in mempool, code execution is halted
  async SlpWaitForTransaction(cashaddr: string, ticker?: string, tokenId?: string): Promise<any> {
    return new Promise(async (resolve) => {
      this.SlpWatchTransactions((data) => {
        resolve(data);
        return true;
  ...
  async SlpWaitForBalance(value: BigNumber.Value, cashaddr: string, ticker: string, tokenId?: string): Promise<SlpTokenBalance> {
    return new Promise(resolve =>
      this.SlpWatchBalance((balance: SlpTokenBalance[]) => {
        let bal = balance[0];
  ...
  SlpWatchBalance(callback: SlpWatchBalanceCallback, cashaddr: string, ticker?: string, tokenId?: string): SlpCancelWatchFn {
    const cancelFn = this.SlpWatchTransactions(() => {
      this.SlpAddressTokenBalances(cashaddr, ticker, tokenId).then(balance => { if (!!callback(balance)) cancelFn(); });
    }, cashaddr, ticker, tokenId);
    return cancelFn;
  }
  ...
  SlpWatchTransactions(callback: SlpWatchTransactionCallback, cashaddr: string, ticker?: string, tokenId?: string): SlpCancelWatchFn {
    const eventSource = this.SlpSocketEventSource(SlpWaitForTransactionTemplate(cashaddr, ticker, tokenId));
  ...
  private SlpDbQuery(queryObject: any): Promise<SlpDbResponse> {
    return new Promise((resolve, reject) => {
      const url = `${servers[this.network].slpdb}/q/${B64QueryString(queryObject)}`;
      fetch_retry(url)
        .then((response: any) => {
  ...
  private SlpSocketEventSource(queryObject: any): EventSource {
    const url = `${servers[this.network].slpsockserve}/s/${B64QueryString(queryObject)}`;
